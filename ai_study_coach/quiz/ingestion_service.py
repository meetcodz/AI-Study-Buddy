...
4. ABSOLUTE CONSTRAINT: The questions MUST test the underlying educational concepts, facts, or domain knowledge learned from the text. You MUST NOT ask questions about the document's structure, layout, or meta-information (e.g., do NOT ask what a specific table shows, what page something is on, or reference the "study material").
5. ABSOLUTE CONSTRAINT: Questions must be ENTIRELY self-contained. The user taking the quiz has NO access to the study material or its figures/tables. NEVER use phrases like "According to the text", "In the example", "As stated above", "In the provided study material", or refer to specific tables/figures. Contextualize the question fully using only the domain knowledge.
...
def process_pdf_and_save(
    ...
    Returns a list of IDs of the saved questions.
    ...
    for i, chunk in enumerate(chunks, 1):
        ...
        try:
            ...
            for q_idx, q in enumerate(questions, 1):
                ...
                chunk_saved += 1
                saved_ids.append(question_obj.id)

            print(f"[CHUNK {i}] ✅ Saved {chunk_saved} question(s) to DB (total so far: {len(saved_ids)})")
    ...
    print(f"\n{'='*60}")
    print(f"[PIPELINE] ✅ DONE — {len(saved_ids)} questions saved to DB.")
    logger.info("Ingestion complete: %d questions saved to DB.", len(saved_ids))
    return saved_ids
