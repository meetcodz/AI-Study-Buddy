...
export default function Quiz({ quizLength = 10, topic = null, onBack, onFinish }) {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/api/sessions/start/`, {
      ...
      .then((sessionData) => {
        ...
        // 2. Fetch questions (filter by topic if provided)
        let questionsUrl = `${API_BASE}/api/questions/?limit=${quizLength}`;
        if (topic) questionsUrl += `&topic=${encodeURIComponent(topic)}`;
